...
public class AiServiceImpl implements AiService {
    ...
    public GenResultVO genResult(String goal, String chartType, String data) {
        ...
        ThrowUtils.throwIf(results.length != 3, ErrorCode.OPERATION_ERROR, "AI服务调用失败，请检查输入是否正确！");
        String resultStr = results[1].trim();

        // 提取&之间的内容
        String[] options = result.split("&");
        ThrowUtils.throwIf(options.length != 2, ErrorCode.OPERATION_ERROR, "AI服务调用失败，请检查输入是否正确！");
        String optionStr = options[1].trim();
        return new GenResultVO(resultStr, optionStr);
    }
}
